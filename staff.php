...
if (ereg('[^+0-9]', $mobile)) {
  ...
  $mobile="$mobile";    
	...
	if ($_POST['empid'] == ""){
		$err = $err . "<br>Staff ID is Null !";
	}
	
	if ($empname == ""){
		$err = $err . "<br>Staff Name is Null";
	}
...
if ($err != ""){
    ...
    $empid = $_POST['empid'];
    $query = mysql_query("select * from staff where stfid='$empid'");
    $numRows = mysql_num_rows($query);
    
    if ($numRows == 0){

		mysql_query("insert into staff(stfid,stfname,dob,gender,status,address,mobile,blood,email,password,image) values('$empid','$empname','$dob','$gender','$status','$address','$mobile','$blood','$email','$password','$image')");
		
        $lastid = mysql_insert_id();
        echo "<script language=javascript>window.location='staffing.php?id=$lastid';</script>";
...
<form name="personal" method="post" action="staff.php">
...
<td height="423" colspan="5" align="center" valign="top" bgcolor="#EDEDED">
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="1">
          <tr>
            <td height="20" bgcolor="#8C8989"><span class="style1"><strong>Staff Personal Details</strong></span></td>
          </tr>
      </table>
<? if ($_SESSION['utype'] == "Administrator"){?>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0">
          <tr>
            <td width="28%" height="30" style="padding:4px;">Staff ID | Username</td>
