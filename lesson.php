...
//$password = createRandomPassword();
	...
	$id = $_GET['id'];
	if ($id != ""){
		$query = mysql_query("select id,stdid,stdname,dept,semt from college where id=$id");
...
if (isset($_POST['Submit'])){
	...
	if ($empid == "" || $empname == "" || $designation == "" || $role == "" || $workdays == "" || $hr == "" || $basicpay == "" || $_POST['other'] == "" ){
	...
	$err = "<font color=red><b>Unable to proceed next step.</b><br> Please Check the following field(s)<br>------------------------------------------------</font>";
		...
		if ($designation == ""){
			$err = $err . "<br> Departement field is empty";
		}
		if ($role == ""){
			$err = $err . "<br> Faculity field is empty";
		}
		if ($workdays == ""){
			$err = $err . "<br> Study Days Per week field is empty";
			}
		if ($hr == ""){
			$err = $err . "<br> Study Hours Per Week field is empty";
		
		}
		if ($bascipay == ""){
			$err = $err . "<br> College Fees Payment field is empty";
...
if ($err != ""){
    ...
    if ($numRows == 0){
		...
		mysql_query("insert into lesson (id,stdid ,stdname ,dept ,fac ,workdays ,numb ,basicpay,other )values($id,'$empid','$empname','$designation','$role','$workdays','$hr','$basicpay','$other')");
...
<td height="423" colspan="5" align="center" valign="top" bgcolor="#EDEDED">
        <table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="1">
          <tr>
            <td height="20" bgcolor="#8C8989"><span class="style1"><strong>Student Lessons Details</strong></span></td>
          </tr>
      </table>
        <? if ($err != ""){ ?>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0">
		 ...
		 <tr>
          ...
          <tr>
            <td height="30" align="left" valign="top" style="padding:4px;">Semester</td>
            <td height="30" style="padding:4px;"><input type="text" name="role" value="<?=$role?>" id="role"></td>
          </tr>
		  
		  		   <tr>
            <td height="30" style="padding:4px;">StudyingDays Per Week</td>
            <td height="30" style="padding:4px;"><select name="workdays" id="workdays">
              <? if ($_POST['workdays'] == "Select" || $_POST['workdays'] == ""){?>
              ...
              <? }
			  elseif($_POST['workdays'] == "1 to 3 days"){
			  ?>
             ...
             <option value="1 to 3 days" selected>1 to 3 days</option>
			  ...
			  <option value="5 to 7 days">5 to 7 days</option>
              <? }
			  elseif($_POST['workdays'] == "3 to 5 days"){
			  ?>
              <option value="1 to 3 days" >1 to 3 days</option>
              <option value="3 to 5 days" selected>3 to 5 days</option>
			  <option value="5 to 7 days">5 to 7 days</option>
              <? }
			  elseif($_POST['workdays'] == "5 to 7 days"){
			  ?>
              <option value="1 to 3 days" >1 to 3 days</option>
              <option value="3 to 5 days" >3 to 5 days</option>
		   ...
		   <tr>
            ...
            <td height="30" style="padding:4px;"><select name="basicpay" id="basicpay">
              <? if ($_POST['basicpay'] == "Select" || $_POST['basicpay'] == ""){?>
              <option selected>Select</option>
              <option value="SFAR">SFAR</option>
              <option value="Private">Private</option>
			  <option value="Other Payment">Other Sponsor</option>
              <? }
			  elseif($_POST['basicpay'] == "SFAR"){
			  ?>
              <option value="SFAR" selected>SFAR</option>
              <option value="Private">Private</option>
			  <option value="Other Payment">Other Payment</option>
              <? }
			  elseif($_POST['basicpay'] == "Private"){
			  ?>
             <option value="SFAR">SFAR</option>
              <option value="Private" selected>Private</option>
			  <option value="Other Payment">Other Payment</option>
              <? }
			  elseif($_POST['basicpay'] == "Other Payment"){
			  ?>
              <option value="SFAR">SFAR</option>
              <option value="Private">Private</option>
			  <option value="Other Payment" selected>Other Payment</option>
              <? }?>
          ...
          <tr>
            <td height="30" style="padding:4px;">&nbsp;</td>
            <td height="30" style="padding:4px;"><input type="submit" name="Submit" id="Submit" onClick="document.personal.empid.disabled=false;" value="Next Step"></td>
          </tr>
        </table>
