...
$query = mysql_query("select stfid,stfname,email from staff where id=$id ");
...
if ($_SESSION['utype'] == "staff") {
	...
	if (isset($_POST['Submit'])){
		...
		if ($subject == "" || $content == ""){
			...
			mysql_query("insert into staffmsg(stfid,stfname,email,subject,content,hit) values('$stfid','$stfname','$email','$subject','$content',0)");
			$subject = "";
			$content = "";
		}
...
<form name="feedback" method="post" action="stfmsg2.php">
...
<td height="423" colspan="5" align="center" valign="top" bgcolor="#EDEDED">
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="1">
          <tr>
            <td height="20" bgcolor="#8C8989" style="padding-left:6px; color:#FFFFFF; font-family:Arial; font-weight:bold;">Comment? Question? Feedback? Opinion? Send Messages to the administrator.</td>
          </tr>
      </table>
      ...
      <? if($_SESSION['utype'] == "staff") {?>
        ...
        <div align="center" style="padding:2px;">
          <div align="center" style=" padding:10px; color:#FFFFFF; font-weight:bold; background-color:#0099FF; border: outset 2px #0000FF; border-width:medium; border-color:#003399;">MESSAGE SENT SUCCESSFULLY! <BR>CHECK YOUR EMAIL INBOX SOON. YOU WILL FIND OUR FEEDBACK!</div>
        </div>
        <? }?>
        <table width="100%" border="0" cellpadding="0" cellspacing="0">
          <tr>
            <td width="28%" height="30" style="padding:4px;">Staff ID</td>
            <td width="72%" height="30" style="padding:4px; font-family:Arial, Helvetica, sans-serif; font-weight:bold; color:#FF0000;"><?=$stfid?><input type="hidden" value="<?=$stdid?>" name="stfid" id="stfid"></td>
          </tr>
          <tr>
            <td height="30" style="padding:4px;">Staff Name</td><td height="30" style="padding:4px; font-family:Arial, Helvetica, sans-serif; font-weight:bold; color:#FF0000;"><?=$stfname?><input type="hidden" value="<?=$stfname?>" name="stdname" id="stdname"></td>
          </tr>
          <tr>
		...
		$query = mysql_query("select * from staffmsg order by ID Desc");
		$numRows = mysql_num_rows($query);
    ...
    <table width="100%"  border="1" cellpadding="0" cellspacing="0" style=" border-collapse:collapse; border-color:#009999">
              <tr style="font-weight:bold;background-color:#9AB9DC">
                <td width="15%" height="25" align="center" valign="middle" style="padding:4px;">Staff ID</td>
                <td width="30%" height="25" align="center" valign="middle">Staff Name</td>
                <td width="55%" height="25" align="center" valign="middle">Subject</td>
              </tr>
              <? while($res = mysql_fetch_array($query)){ ?>
              <tr>
                <td height="20" align="left" valign="middle" style="padding:4px;"><?=$res['stfid']?></td>
                <td height="20" align="left" valign="middle" style="padding:4px;"><?=$res['stfname']?></td>
                <td height="20" align="left" valign="middle" style="padding:4px;">
				<? if ($res['hit'] == 0){ ?>
					<a href="viewstfmsg.php?id=<?=$res['ID']?>"><b><?=$res['subject']?></b></a>
				<? } else{?>
					<a href="viewstfmsg.php?id=<?=$res['ID']?>"><?=$res['subject']?></a>
                <? }?>
                </td>
              </tr>
              <? }?>
